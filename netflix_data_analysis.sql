...
SELECT 
	release_year,
	title
	-- RANK(*) OVER(ORDER BY title) AS "Sr.No." 
FROM netflix
WHERE release_year = 2020
	and "type" = 'Movie';
-- ORDER BY RANK(*) OVER(ORDER BY title) DESC;
...
SELECT 
	...
	ROUND(COUNT(show_id)*100/(SELECT COUNT(*) FROM netflix WHERE country Like '%India%'),0) AS avg_content
FROM CTE
GROUP BY 1
ORDER BY 3 DESC;
